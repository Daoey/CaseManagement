...
public final class TestSqlTeamRepository {
    ...
    public void addUserToTeam() throws RepositoryException {
        ...
        User ourUser = userRepository.getUserById(userId);
        List<User> usersInTeamWithId1 = userRepository.getUsersByTeamId(teamId);
        
        assertFalse(listContainUserWithId(usersInTeamWithId1, userId));
        
        sqlTeamRepository.addUserToTeam(ourUser.getId(), teamId);

        usersInTeamWithId1 = userRepository.getUsersByTeamId(teamId);
        
        assertTrue(listContainUserWithId(usersInTeamWithId1, userId));
    }

    private boolean listContainUserWithId(List<User> users, int userId) {
        for (User user : users)
